...
fn create_file_response(ts_file: &TSFile) -> Result<FileResponse, String> {
  ...
  let package_declaration_node =
    crate::commands::java::treesitter::services::package_declaration_service::get_package_declaration_node(ts_file);
  let package_name = package_declaration_node
    .and_then(|node| {
      crate::commands::java::treesitter::services::package_declaration_service::get_package_scope_node(ts_file, node)
    })
