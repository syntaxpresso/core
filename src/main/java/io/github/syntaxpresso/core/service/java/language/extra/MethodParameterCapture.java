 ...
 */
public enum MethodParameterCapture {
  ...
  MethodParameterCapture(String captureName) {
  ...
  /**
   * Find a MethodParameterCapture by its capture name.
   *
   * @param captureName the name to search for (with or without @ prefix)
   * @return the matching MethodParameterCapture, or null if not found
   */
  public static MethodParameterCapture fromCaptureName(String captureName) {
    if (captureName == null) {
      return null;
    }
    ...
    for (MethodParameterCapture capture : values()) {
