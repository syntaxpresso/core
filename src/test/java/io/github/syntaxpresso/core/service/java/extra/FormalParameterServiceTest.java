...
class FormalParameterServiceTest {
  ...
  void setUp() {
    ...
    formalParameterService = programService.getClassDeclarationService().getMethodDeclarationService().getFormalParameterService();
  ...
  class FindAllFormalParametersTests {
    ...
    void shouldFindFormalParametersOfSpecificType() {
      ...
      List<TSNode> calculatorParams = formalParameterService.findAllFormalParameters(testFile, processCalculatorMethod, "Calculator");
      assertEquals(1, calculatorParams.size(), "Should find 1 Calculator parameter in processCalculator");
      
      List<TSNode> stringParams = formalParameterService.findAllFormalParameters(testFile, processCalculatorMethod, "String");
    ...
    void shouldFindMultipleParametersOfSameType() {
      ...
      List<TSNode> calculatorParams = formalParameterService.findAllFormalParameters(testFile, testMethod, "Calculator");
    ...
    void shouldReturnEmptyListForNonExistentType() {
      TSNode processCalculatorMethod = findMethodByName("processCalculator");
      List<TSNode> nonExistentParams = formalParameterService.findAllFormalParameters(testFile, processCalculatorMethod, "NonExistent");
    ...
    void shouldHandleInvalidMethodNode() {
      ...
      List<TSNode> params = formalParameterService.findAllFormalParameters(testFile, classNodes.get(0), "Calculator");
    ...
    void shouldHandleNullParameters() {
      ...
      List<TSNode> nullNodeResult = formalParameterService.findAllFormalParameters(testFile, null, "Calculator");
      assertTrue(nullNodeResult.isEmpty(), "Should handle null method node");
      
      List<TSNode> nullFileResult = formalParameterService.findAllFormalParameters(null, method, "Calculator");
  ...
  class GetParameterTypeNodeTests {
    ...
    void shouldFindTypeNodeInFormalParameter() {
      ...
      Optional<TSNode> typeNode = formalParameterService.getParameterTypeNode(calculatorParam, testFile, "Calculator");
    ...
    void shouldReturnEmptyForNonMatchingType() {
      ...
      Optional<TSNode> typeNode = formalParameterService.getParameterTypeNode(stringParam, testFile, "Calculator");
    ...
    void shouldHandleInvalidParameterNode() {
      List<TSNode> classNodes = testFile.query("(class_declaration) @class");
      Optional<TSNode> typeNode = formalParameterService.getParameterTypeNode(classNodes.get(0), testFile, "Calculator");
    ...
    void shouldHandleNullParameters() {
      TSNode processCalculatorMethod = findMethodByName("processCalculator");
      List<TSNode> params = formalParameterService.findAllFormalParameters(testFile, processCalculatorMethod, "Calculator");
      TSNode param = params.get(0);
      
      assertFalse(formalParameterService.getParameterTypeNode(null, testFile, "Calculator").isPresent());
      assertFalse(formalParameterService.getParameterTypeNode(param, null, "Calculator").isPresent());
  ...
  class GetParameterNameNodeTests {
    ...
    void shouldExtractParameterNameFromFormalParameter() {
      ...
      TSNode calculatorParam = calculatorParams.get(0);
      Optional<TSNode> nameNode = formalParameterService.getParameterNameNode(calculatorParam, testFile);
    ...
    void shouldHandleDifferentParameterTypes() {
      ...
      TSNode stringParam = stringParams.get(0);
      Optional<TSNode> nameNode = formalParameterService.getParameterNameNode(stringParam, testFile);
    ...
    void shouldHandleInvalidParameterNode() {
      List<TSNode> classNodes = testFile.query("(class_declaration) @class");
      Optional<TSNode> nameNode = formalParameterService.getParameterNameNode(classNodes.get(0), testFile);
  ...
  class IsFormalParameterDeclarationTests {
    ...
    void shouldHandleNullAndInvalidNodes() {
      assertFalse(formalParameterService.isFormalParameterDeclaration(null), "Should handle null node");
      
      // Create a mock null node scenario
      List<TSNode> classNodes = testFile.query("(class_declaration) @class");
      TSNode classNode = classNodes.get(0);
      assertFalse(formalParameterService.isFormalParameterDeclaration(classNode), "Should handle non-identifier node");
    }
  ...
  class FindAllFormalParameterUsagesTests {
    ...
    void shouldFindFormalParameterUsagesInMethodBody() {
      TSNode processCalculatorMethod = findMethodByName("processCalculator");
      List<TSNode> usages = formalParameterService.findAllFormalParameterUsages(testFile, processCalculatorMethod, "Calculator");
      ...
      assertTrue(foundCompute || foundAssignment, "Should find at least one of the expected usages");
    }
    ...
    void shouldFindMultipleParameterUsages() {
      TSNode testMethod = findMethodByName("testMethod");
      List<TSNode> usages = formalParameterService.findAllFormalParameterUsages(testFile, testMethod, "Calculator");
      ...
      long primaryUsages = usages.stream()
          .map(node -> testFile.getTextFromNode(node))
          .filter("primary"::equals)
          .count();
      
      long secondaryUsages = usages.stream()
          .map(node -> testFile.getTextFromNode(node))
          .filter("secondary"::equals)
          .count();
      
      assertTrue(primaryUsages > 0 || secondaryUsages > 0, "Should find usages of primary or secondary parameters");
    }
    ...
    void shouldReturnEmptyListForNonExistentParameterType() {
      ...
      List<TSNode> usages = formalParameterService.findAllFormalParameterUsages(testFile, noMatchingMethod, "Calculator");
      
      assertTrue(usages.isEmpty(), "Should return empty list when no parameters of specified type exist");
    }
    ...
    void shouldHandleInvalidMethodNode() {
      List<TSNode> classNodes = testFile.query("(class_declaration) @class");
      List<TSNode> usages = formalParameterService.findAllFormalParameterUsages(testFile, classNodes.get(0), "Calculator");
    ...
    void shouldHandleNullParameters() {
      ...
      List<TSNode> nullMethodResult = formalParameterService.findAllFormalParameterUsages(testFile, null, "Calculator");
      assertTrue(nullMethodResult.isEmpty(), "Should handle null method node");
      
      List<TSNode> nullFileResult = formalParameterService.findAllFormalParameterUsages(null, method, "Calculator");
      assertTrue(nullFileResult.isEmpty(), "Should handle null file gracefully");
    }
  ...
  class IntegrationTests {
    ...
    void shouldWorkTogetherToAnalyzeParameterInformation() {
      ...
      for (TSNode param : calculatorParams) {
        Optional<TSNode> typeNode = formalParameterService.getParameterTypeNode(param, testFile, "Calculator");
        ...
        assertTrue(List.of("primary", "secondary").contains(paramName), 
    ...
    void shouldHandleComplexParameterScenarios() {
      ...
      List<TSNode> calculatorParams = formalParameterService.findAllFormalParameters(complexFile, complexMethod, "Calculator");
