...
class TSQueryBuilderTest {
  ...
  @BeforeEach
  void setUp() {
    this.tsFile = new TSFile(SupportedLanguage.JAVA, JAVA_CODE);
    this.setupTestNodes();
  }

  private void setupTestNodes() {
    TSNode rootNode = this.tsFile.getTree().getRootNode();
    this.classNode = this.findNodeByType(rootNode, "class_declaration");
    this.getNameMethod = this.findMethodByName("getName");
  }
  ...
  class ScopedQueryTests {
    ...
    void shouldExecuteQueryWithinSpecificNodeScope() {
      TSQueryResult result =
          tsFile.query("(method_declaration) @method").within(classNode).execute();

      assertEquals(4, result.size());
      assertTrue(result.nodes().stream().allMatch(node -> isNodeWithin(node, classNode)));
    }
    ...
    void shouldLimitResultsToScope() {
      ...
      TSQueryResult result =
          tsFile.query("(method_declaration) @method").within(getNameMethod).execute();
    ...
    void shouldChainWithinCalls() {
      TSQueryBuilder builder = tsFile.query("(identifier) @id").within(classNode);
  ...
  class SingleCaptureReturnTests {
    ...
    void shouldReturnSpecificCapture() {
      ...
      assertTrue(nameNodes.stream().allMatch(node -> "identifier".equals(node.getType())));
    }
    ...
    void shouldHandleNonExistentCaptureName() {
      TSQueryResult result =
          tsFile.query("(method_declaration) @method").returning("nonexistent").execute();
    ...
    void shouldChainReturningCall() {
      TSQueryBuilder builder =
          tsFile.query("(method_declaration name: (identifier) @name)").returning("name");
  ...
  class FilteredCapturesReturnTests {
    ...
    void shouldFilterOutEmptyCaptureMaps() {
      TSQueryResult result =
          tsFile.query("(method_declaration) @method").returningCaptures("nonexistent").execute();
  ...
  class PredicateQueryTests {
    ...
    void shouldEvaluateMatchPredicates() {
      ...
      if (result.size() > 0) {
        ...
        assertTrue(methods.stream().allMatch(node -> "method_declaration".equals(node.getType())));
      }
    }
  ...
  class ComplexQueryTests {
    ...
    void shouldHandleNestedQueriesWithinScopedNodes() {
      TSQueryResult result = tsFile.query("(identifier) @id").within(getNameMethod).execute();
  ...
  class MethodChainingTests {
    ...
    void shouldSupportFullMethodChaining() {
      ...
      assertEquals(4, result.size());
      assertTrue(result.nodes().stream().allMatch(node -> "identifier".equals(node.getType())));
    }

    @Test
    @DisplayName("Should allow chaining in different orders")
    void shouldAllowChainingInDifferentOrders() {
      TSQueryResult result1 =
          tsFile
  ...
  class ErrorHandlingTests {
    ...
    void shouldHandleEmptyQueryString() {
      TSQueryResult result = tsFile.query("").execute();
    ...
    void shouldHandleQueriesWithSyntaxErrors() {
      TSQueryResult result = tsFile.query("(invalid syntax here").execute();
