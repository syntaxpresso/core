...
class TSFileExtendedTest {
  ...
  class FindParentNodeByTypeTests {
    ...
    void shouldHandleDeeplyNestedStructures() {
      ...
      // Find parent method
      Optional<TSNode> parentMethod = complexFile.findParentNodeByType(stringLiteral, "method_declaration");
      assertTrue(parentMethod.isPresent(), "Should find parent method");
      
      // Find parent class
      Optional<TSNode> parentClass = complexFile.findParentNodeByType(stringLiteral, "class_declaration");
      assertTrue(parentClass.isPresent(), "Should find parent class");
    }
    ...
    void shouldReturnEmptyWhenParentTypeNotFound() {
      ...
      Optional<TSNode> parentInterface = testFile.findParentNodeByType(classNode, "interface_declaration");
    ...
    void shouldHandleTSExceptionGracefully() {
      ...
      Optional<TSNode> result = testFile.findParentNodeByType(deepNode, "compilation_unit");
    ...
    void shouldFindImmediateParentCorrectly() {
      ...
      Optional<TSNode> parentDeclarator = testFile.findParentNodeByType(identifierNode, "variable_declarator");
    ...
    void shouldTraverseMultipleLevelsCorrectly() {
      ...
      // Should be able to find local_variable_declaration (grandparent)
      Optional<TSNode> localVarDecl = testFile.findParentNodeByType(identifierNode, "local_variable_declaration");
      assertTrue(localVarDecl.isPresent(), "Should find local_variable_declaration ancestor");
      
      // Should be able to find method_declaration (great-grandparent)
      Optional<TSNode> methodDecl = testFile.findParentNodeByType(identifierNode, "method_declaration");
  ...
  class ErrorResilienceTests {
    ...
    void shouldMaintainFunctionalityAfterModifications() {
      ...
      List<TSNode> methods = testFile.query("(method_declaration) @method");
      if (!methods.isEmpty()) {
        Optional<TSNode> parentClass = testFile.findParentNodeByType(methods.get(0), "class_declaration");
